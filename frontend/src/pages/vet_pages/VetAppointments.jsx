...
const VetAppointments = () => {
  ...
  const [formData, setFormData] = useState({
    pet: "", date: "", time: "", reason: "", status: "pending", notes: ""
  });
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const response = await api.get(`/vet/appointments/${query ? `?${query}` : ''}`);
  ...
  const statusBadge = (status) => {
    ...
    return <span className={`px-2 py-1 rounded-full text-xs font-medium ${map[status]}`}>{labels[status] || status}</span>;
  };

  const handleAction = async (id, action) => {
    try {
      await api.patch(`/vet/appointments/?id=${id}`, { status: action });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Supprimer ce rendez-vous ?")) {
      try {
        await api.delete(`/vet/appointments/?id=${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingAppt) {
        await api.patch(`/vet/appointments/?id=${editingAppt.id}`, formData);
      } else {
        await api.post('/vet/appointments/', formData);
      }
    ...
    <VetLayout>
      <div className="p-10">
        <div className="mb-6 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold mb-2 text-[#8657ff]">Gestion des Rendez-vous</h1>
                    ...
                    <tr key={appt.id} className="border-t border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="p-4">
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => openForm(appt)} className="text-green-600 hover:text-purple-800 p-1" title="Modifier"><Edit className="w-4 h-4" /></button>
                          {appt.status === "pending" && (
                            <button onClick={() => handleAction(appt.id, "confirmed")} className="text-green-600 hover:text-purple-800 p-1" title="Confirmer"><CheckCircle className="w-4 h-4" /></button>
                          )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg w-full max-w-lg p-6 relative">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2"><PawPrint className="w-4 h-4 text-gray-500 mt-0.5" /><span><strong>Animal :</strong> {selectedAppt.petName}</span></div>
              ...
              <div className="mt-6 flex gap-3">
                ...
                <button onClick={() => { handleAction(selectedAppt.id, "completed"); setShowModal(false); }} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-lg">Marquer terminé</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg w-full max-w-2xl p-6 relative max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                ...
                <input type="time" value={formData.time} required onChange={(e) => setFormData({ ...formData, time: e.target.value })}
                  className="p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#8657ff]" />
                <input type="text" placeholder="Motif de la consultation" value={formData.reason} required onChange={(e) => setFormData({ ...formData, reason: e.target.value })}
                  className="col-span-2 p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#8657ff]" />
                <textarea placeholder="Notes (optionnel)" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                  className="col-span-2 p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#8657ff]" rows={3} />
